...
echo "[start.sh] PORT=${PORT:-3001} DB_PATH=${DB_PATH:-<default sklat.db>}"
...
if [ -n "${DB_PATH:-}" ]; then
  mkdir -p "$(dirname "$DB_PATH")" 2>/dev/null || echo "[start.sh] WARN: could not create $(dirname "$DB_PATH")"
fi
...
if "$PYTHON" -c "import asyncio; from database.db import init_db; asyncio.run(init_db())"; then
  ...
  echo "[start.sh] WARN: init_db failed (continuing; web server will report DB errors)"
fi
